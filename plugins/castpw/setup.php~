...
function castpw_process() {
  global $lang, $castpw_countdisabled, $castpw_saved, $spectacleid, $ss;
  $count_saved = 0;
  foreach ($ss as $sh) 
  ...
  if (admin_mode() && isset($_POST["reset-disabled"])) {
    ...
    if ($castpw_countdisabled == 0) {
      $ok = mysql_query("update spectacles set castpw='' where id='$spectacleid'");
    ...
    if (!empty($castpw)) {
      if (admin_mode()) {
        ...
        if ($len < 8) {
          ...
          $res = mysql_query("update spectacles set castpw='$castpw' where id='$spectacleid'");
...
function castpw_display() {
  ...
  if (admin_mode()) {
    ...
    echo "<br /><br />";
    echo "<form action='repr.php?spectacleid=$spectacleid' method='post'>";
