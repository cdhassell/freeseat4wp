 ...
 * PHP-Barcode 0.3pl1
 *
 * (Modified by Leif Harmsen to render codes sideways)
 * (Modified by tendays to remove warnings)
 *
...
/* <tendays> I haven't managed to get this working yet... */

//$font_loc=dirname($_SERVER["PATH_TRANSLATED"])."/fontname.ttf";
...
function barcode_encode_genbarcode($code,$encoding){
    ...
    if (("EAN" == strtoupper($encoding)) && strlen($code)==13) $code=substr($code,0,12);
...
function barcode_encode($code,$encoding){
    global $genbarcode_loc;
    if (
	(( !isset($encoding) || !$encoding || ("ANY" == strtoupper($encoding) ))
		 && (preg_match("/^[0-9]{12,13}$/", $code)))

		|| (($encoding) && ("EAN" == strtoupper($encoding)
		 && ( strlen($code)==12 || strlen($code)==13)))
		 
		|| (($encoding) && ("ISBN" == strtoupper($encoding))
