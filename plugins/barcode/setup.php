...
function barcode_link() {
  global $filtershow;
  if ($filtershow) {
    echo "<ul><li><p class='main'><a href='". plugins_url("index.php?id=$filtershow",__FILE__) . 
    "' target='barcodescanner'>Scan Tickets</a> for this show.</p></ul>";
  }
}

function barcode_ticket_left($booking) {
	echo "<img height=125 src='" . plugins_url("barcode.php?code="
		.str_pad($booking["bookid"],12,"0",STR_PAD_LEFT)
		."&amp;encoding=EAN&amp;scale=2&amp;mode=png", __FILE__) . "'>";
	return false;
}
